...
window.addEventListener('AppReady', async () => {
    ...
    if (SPREADSHEET_ID) {
        ...
        document.getElementById('syncStatus').innerHTML = '<i class="fa-solid fa-database me-1"></i>使用備援資料庫';
    }
...
async function fetchGoogleSheetsData() {
    try {
        ...
        document.getElementById('syncStatus').innerHTML = '<i class="fa-solid fa-circle-check me-1"></i>雲端資料同步完成';
    } catch (err) {
        console.warn("無法動態讀取 Google 試算表，使用預設資料:", err);
        document.getElementById('syncStatus').innerHTML = '<i class="fa-solid fa-triangle-exclamation me-1"></i>同步失敗，切換至備援庫';
    }
}
            ...
            <div class="patent-card">
                <div class="patent-img-wrapper" data-bs-toggle="modal" data-bs-target="#${modalId}">
                    ...
                    <div class="patent-img-overlay">
                        <span class="img-zoom-hint"><i class="fa-solid fa-magnifying-glass-plus me-1"></i>點擊查看證書</span>
                    </div>
                </div>
                <div class="patent-header">
                    ...
                    <div class="patent-no"><i class="fa-solid fa-award me-1"></i>${item.patentNo || '國際專利認證'}</div>
                </div>
                <div class="patent-body">
                    <p class="patent-desc">${item.desc}</p>
                    <ul class="feature-list">
                        ${item.feature1 ? `<li><i class="fa-solid fa-circle-check me-1"></i>${item.feature1}</li>` : ''}
                        ${item.feature2 ? `<li><i class="fa-solid fa-circle-check me-1"></i>${item.feature2}</li>` : ''}
                        ${item.feature3 ? `<li><i class="fa-solid fa-circle-check me-1"></i>${item.feature3}</li>` : ''}
                    </ul>
                </div>
                <div class="patent-footer">
                    <span class="award-badge"><i class="fa-solid fa-trophy me-1"></i>${item.awardBadge || '權威背書'}</span>
                    <button class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#${modalId}"><i class="fa-solid fa-file-contract me-1"></i>查看證書</button>
                </div>
            </div>
        </div>`;
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title text-mint"><i class="fa-solid fa-certificate me-1"></i>${item.title}</h5>
                    ...
                    <div class="modal-body p-4">
                        ...
                        <h6 class="text-warning"><i class="fa-solid fa-trophy me-1"></i>權威認證與亮點：</h6>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"><i class="fa-solid fa-xmark me-1"></i>關閉</button>
                    </div>
                </div>
            </div>
        </div>`;
